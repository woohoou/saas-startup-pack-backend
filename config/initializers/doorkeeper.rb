  ...
  resource_owner_from_assertion do
    if server.client && params[:provider] && params[:assertion]
      ...
      when 'google'
        ...
        user = client.tokeninfo(access_token: params[:assertion]).to_h
      when 'facebook'
        client = Koala::Facebook::API.new(params[:assertion]) 
        user = client.get_object("me", {:fields => 'picture, email, first_name, last_name, birthday'}).with_indifferent_access
      when 'github'
      ...
      if user.present?
        User.from_identity(user.merge(provider: params[:provider]))
      end
    end
  end
